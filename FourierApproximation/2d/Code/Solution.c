...
double FullCompute(double x, int N, double * cNks, double (*u)(double, double), double * netmemory, double * umemory, double * phimemory) 
{
...
void FullUMatrix(double * matrix, int N, double * net, double (*u)(double)) 
{
    for(int i = 1; i < N + 1; ++i) 
    {
        for(int j = 1; j < N + 1; ++j) 
        {
            if(i == 1 || i == N || j == 1 || j == N)  matrix[e(i, j, N)] = 0;
            else matrix[e(i, j, N)] = u(net[i], net[j]);
        }
    }
}